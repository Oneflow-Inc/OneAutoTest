...
def parse_args():
    ...
    parser.add_argument("--saving_path", type=str, default="oneflow-sd-output", help="Directory where the generated images will be saved")
...
if __name__ == "__main__":
    ...
    with torch.autocast("cuda"):
        for j in range(args.num_images_per_prompt):
            print("[oneflow]", "[gpu_memory]", f"{gpu_memory_used()} MiB")
            start = timer()
            images = pipe(
                args.prompt,
            ...
            print("[oneflow]", "[last_gpu_memory]", f"{gpu_memory_used()} MiB")
