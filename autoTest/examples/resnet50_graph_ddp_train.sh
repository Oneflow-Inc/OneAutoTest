...
bash examples/args_train_ddp_graph.sh 1 2 0 127.0.0.1 /ssd/dataset/ImageNet/ofrecord 160 50 true python3 graph gpu 100 false "${NSYS_BIN}" ${RUN_COMMIT}
...
#bash examples/args_train_ddp_graph.sh 1 4 0 127.0.0.1 /ssd/dataset/ImageNet/ofrecord 96 50 false python3 ddp cpu 100 false "${NSYS_BIN}" ${RUN_COMMIT}


#python3 ${SRC_DIR}/OneAutoTest/ResNet50/tools/extract_result.py --model-type ${MODEL_TYPE} --run-type ${RUN_TYPE} --test-commit ${git_commit} --test-log ${MODEL_DIR}/test_logs/$HOSTNAME --compare-commit ${git_commit} --url-path OneBrain/commit/${RUN_COMMIT}/$(date "+%Y%m%d")/${git_commit}/ResNet50-${MODEL_TYPE}/${RUN_TYPE}

#${SRC_DIR}/oss/ossutil64 -c ${SRC_DIR}/oss/ossutilconfig cp -r -f ${MODEL_DIR}/test_logs/$HOSTNAME/1n2g  oss://oneflow-test/autoTest/commit/${RUN_COMMIT}/$(date "+%Y%m%d")/${git_commit}/ResNet50-ddp/${RUN_TYPE}/1n2g/
